...
const fmtNumber=(n,d=2)=>(Number(n)||0).toLocaleString('lv-LV',{
  minimumFractionDigits:d, maximumFractionDigits:d});
const fmtMoney =(n)=>(Number(n)||0).toLocaleString('lv-LV',{
...
function formatRange(a,b){
  const o={day:'2-digit', month:'short'};
    return `${a.toLocaleDateString('lv-LV', o)} – 
            ${b.toLocaleDateString('lv-LV', o)}`;
}
...
function lvHolidaySet(y){
  const s=new Set(),p=(dt)=>s.add(localISO(dt));
        p(new Date(y,0,1));
  const eas=easterSunday(y); 
        p(addDays(eas,-2)); 
        p(eas); p(addDays(eas,1));
        p(new Date(y,4,1)); p(new Date(y,4,4));
        p(new Date(y,5,23)); p(new Date(y,5,24));
        p(new Date(y,10,18)); p(new Date(y,11,24));
        p(new Date(y,11,25)); p(new Date(y,11,26));
        p(new Date(y,11,31));
    return s; 
}
...
function isWorkday(iso){
  ...
  const dow=(d.getDay()+6)%7;
    return dow<=4 && !isHoliday(iso);
}

function loadEntries(){
  try{ 
    return JSON.parse(localStorage.getItem(lsKey)) || []; }catch{ return []; 
          } 
}
...
function updateEntry(id, patch){
  const list=loadEntries().map(e=> e.id===id ?
    ({...e,...patch}) : e);
    saveEntries(list);
}
...
function sumPeriod(entries, startISO, endISO, settings){
  const byDay={}; entries.filter(e=>e.date>=startISO && e.date<=endISO)
    .forEach(e=>{
      byDay[e.date]=byDay[e.date]||[];
      byDay[e.date].push(e); });
  let total=0, normal=0, over=0, amount=0; Object.keys(byDay)
    .forEach(iso=>{
      const t=dayTotals(entries,iso,settings);
        total+=t.hDay; normal+=t.normal; over+=t.over;
        amount+=t.amount; });
...
document.getElementById('homeBtn')?
  .addEventListener('click', ()=> setActiveTab('week'));
document.getElementById('btnWeek')?
  .addEventListener('click', ()=> setActiveTab('week'));
document.getElementById('btnMonth')?
  .addEventListener('click', ()=> setActiveTab('month'));
document.getElementById('btnSettings')?
  .addEventListener('click', ()=> setActiveTab('settings'));
...
function openDaySheet(iso){
  selectedISO=iso; fillDaySheet(iso);
  scrim.hidden=false;
  sheet.hidden=false;
...
scrim?.addEventListener('click', closeDaySheet);
  sheetClose?.addEventListener('click', closeDaySheet);
  sheetCancel?.addEventListener('click', closeDaySheet);
  ...
  if(t.rows.length){
    t.rows.forEach(r=>{
      ...
      row.querySelector('[data-act="del"]').addEventListener('click', ()=>{
        if(confirm('Dzēst ierakstu?')){ deleteEntry(r.id); renderWeek(); renderMonth(); fillDaySheet(iso); } });
      row.querySelector('[data-act="edit"]').addEventListener('click', ()=>{
        ...
        const hh=parseNum(newH);
          if(hh<=0) 
            return alert('Nederīgs skaitlis');
    ...
    const empty=document.createElement('div');
      ...
      empty.innerHTML=`<div class="small">Nav ierakstu šai dienai.</div>`;
        sheetEntries.appendChild(empty);
      }
...
sheetAdd?.addEventListener('click', ()=>{
  if(!selectedISO)
    return;
      const hh=parseNum(sheetHours.value);
  if(hh<=0)
    return alert('Ievadi derīgas stundas');
  const s=loadSettings();
        addEntry({ id:'e_'+Date.now()+'_'+Math.random().toString(36).slice(2), date:selectedISO, hours:hh,
    activity:(sheetActivity.value??'').trim(),
...
nextWeekBtn?.addEventListener('click', ()=>{
  const [mon]=weekBounds(new Date(currentWeekAnchor));
        mon.setDate(mon.getDate()+7); currentWeekAnchor=mon;
  renderWeek(); });
...
function renderWeek(){
  ...
  const [ws,we]=weekBounds(currentWeekAnchor);
    weekNoEl.textContent=isoWeekNumber(ws);
    weekRangeEl.textContent=formatRange(ws,we);
  const totals=sumPeriod(entries, localISO(ws),
                         localISO(we), settings);
    ...
    leftColEl.innerHTML=''; rightColEl.innerHTML='';
      for(let i=0;i<7;i++){
        ...
        const t=dayTotals(entries,iso,settings);
          ...
          else if(t.workday){ if(t.hDay<8) bg='bg-blue';
        ...
        const label=d.toLocaleDateString('lv-LV',{
          ...
          card.innerHTML=`<div class="day-head">
              <div class="day-date ${t.holiday?'holiday':''}">${label}
                </div>
              <div class="badge">${fmtNumber(t.hDay,2)} h
                </div>
              </div>`;
          if(t.rows.length){ t.rows.forEach(r=>{
            const row=document.createElement('div');
              row.className='entry';
              row.innerHTML=`<div class="entry-line"><div><strong>${
                fmtNumber(r.hours,2)} h</strong>${r.activity?` · <span class=\"note\">${escapeHtml(r.activity)}</span>`:''}</div></div>`; card.appendChild(row); }); }
          else{ const empty=document.createElement('div');
               empty.className='entry';
               empty.innerHTML=`<div class="small">Nav ierakstu šai dienai.</div>`;
          ...
          const bottom=document.createElement('div');
                ...
                bottom.textContent=`Obligātās: ${fmtNumber(t.normal,2)} h · Virsstundas: ${fmtNumber(t.over,2)} h · Bruto: ${fmtMoney(t.amount)}`; card.appendChild(bottom);
...
prevMonthBtn?.addEventListener('click', ()=>{
  currentMonthAnchor=new Date(
    currentMonthAnchor.getFullYear(),
    currentMonthAnchor.getMonth()-1,1);
  renderMonth(); });
...
document.getElementById('openPrint')?
        .addEventListener('click', ()=>{
          const y=currentMonthAnchor.getFullYear();
          const mm=String(currentMonthAnchor.getMonth()+1).padStart(2,'0');
            window.open(`./print.html?month=${y}-${mm}`,'_blank','noopener,noreferrer'); });

function countWorkdaysInMonth(year,mi){
  const s=new Date(year,mi,1),e=new Date(year,mi+1,0);
    let c=0; for(let d=new Date(s); d<=e; d.setDate(d.getDate()+1)){
      if(isWorkday(localISO(d))) c++; }
  return c;
}
  
function remainingWorkdaysFromToday(year,mi){
  const now=new Date(); if(now.getFullYear()!==year||now.getMonth()!==mi)
    return 0;
  const s=new Date(now.getFullYear(),now.getMonth(),now.getDate()),e=new Date(year,mi+1,0);
    let c=0; 
...
function renderMonth(){
  ...
  const startISO=localISO(ms), endISO=localISO(me);
  const totals=sumPeriod(entries,startISO,endISO,settings);
  const y=ms.getFullYear(), mi=ms.getMonth();
  const workdays=countWorkdaysInMonth(y,mi), required=workdays*8;
  ...
  const byDay={}; entries.filter(e=> e.date>=startISO && e.date<=endISO)
    .forEach(e=>{ byDay[e.date]=byDay[e.date]||[];
                 byDay[e.date].push(e); });
  const days=Object.keys(byDay).sort();
    ...
    if(days.length===0){
      const empty=document.createElement('div');
        ...
        empty.innerHTML=`
          <div class="small">Šim mēnesim nav ierakstu.</div>
          <div class="right"></div>`;
      ...
      return; }
        days.forEach(iso=>{
          ...
          const row=document.createElement('div');
            ...
            row.innerHTML=`
              <div><strong class="${isHoliday(iso)?'holiday':''}">${label}</strong>
              <div class="small">${t.rows.length} ieraksti · ${
                fmtNumber(t.hDay,2)} h · Obligātās ${
                fmtNumber(t.normal,2)} h · Virsst. ${
                fmtNumber(t.over,2)} h</div>
                </div>
              <div class="right"><strong>${
                fmtMoney(t.amount)}</strong></div>`;
  ...
  const s=loadSettings();
    ...
    settingsForm?.addEventListener('submit',(e)=>{ e.preventDefault();
        ...
        const rateWeekend=rateWeekendEl.value.trim()===''?null:parseNum(rateWeekendEl.value);
            if(rate<=0||rateOver<=0||thr<=0)
              return alert('Pārbaudi iestatījumu vērtības');
            if(rateWeekend!=null&&rateWeekend<=0)
              return alert('Brīvdienu likmei jābūt pozitīvai');
...
(function init(){
  ...
  const [mon]=weekBounds(new Date());
    window.currentWeekAnchor=mon;
    window.currentMonthAnchor=new Date(new Date().getFullYear(),
                                       new Date().getMonth(), 1);
    renderSettings();
    setActiveTab('week'); })();
})();
